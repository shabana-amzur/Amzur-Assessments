export default async function handler(request, response) {
  ...
  const { email, industry, score, tier, dimensions, ranked = Object.keys(dimensions || {}) } = request.body || {}
  ...
  try {
    const [userResponse, notificationResponse] = await Promise.all([
      ...
      sendEmail({ to: [notificationEmail], subject: `New ${assessmentName} completed - ${industry}`, html: notificationHtml }),
    ])
